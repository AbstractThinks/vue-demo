<template>
  <div id="recommendmorning">
    ...
    <mu-popup position="top" popupClass="consultation-popup" :open="topPopup" @close="close('top')">
        ...
        <mu-flat-button label="我知道了" fullWidth @click="close('top')" primary/>
    </mu-popup>
    <mu-popup position="top" popupClass="consultation-popup" :open="topRulePopup" @close="close('topRule')">
        ...
        <mu-flat-button label="我知道了" fullWidth @click="close('topRule')" primary/>
    </mu-popup>
    ...
    <mu-content-block class="relative">
      ...
      <div v-html="recommendmor.firstData?recommendmor.firstData.sremark:''"></div>
      <div class="blank20"></div>
      
      <div class="text-center">
          <mu-flat-button label="查看历史表现回顾 " labelPosition="before" icon=":iconfont icon-jiantou" primary @click="toggleHistory()" iconClass="font14">
            <!-- <mu-icon slot="right" value=":iconfont icon-jiantou"/> -->
          </mu-flat-button>
      </div>
      <mu-list  v-if="toggleHistoryShow" class="zoom-in">
